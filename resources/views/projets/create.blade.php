...
<div class="w-[80%] mx-auto px-6 py-10 bg-white dark:bg-gray-900 rounded-lg shadow-lg">
    ...
    <form action="{{ route('projets.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nom" value="{{ old('nom') }}" required class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
            <textarea name="description" required rows="4" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description') }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date de début</label>
                <input type="date" name="date_debut" value="{{ old('date_debut') }}" required class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date de fin</label>
                <input type="date" name="date_fin" value="{{ old('date_fin') }}" required class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
        </div>
        ...
        <div id="tacheSection" class="hidden">
            <div id="tacheContainer" class="flex flex-wrap gap-6 justify-center">
                <div class="tacheItem w-full md:w-[30%] border p-4 rounded-lg dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    ...
                    <div class="mt-4 text-right">
                        <button type="button" class="deleteTache bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700">
